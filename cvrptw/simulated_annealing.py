...
def sa_algorithm(instance, temp_start = 350, update_temp = lambda t : 0.9999 * t, stop_criterion = lambda t : t <= 0.01):
    ...
    temp_set = np.array(temp)
    dist_set = np.array(curr_dist)
    ...
    while not stop_criterion(temp):
        ...
        dist_set = np.append(dist_set, curr_dist) 
    ...
    return [(afterOneMin, cOneMin), (afterFiveMin, cFiveMin), (incumb_solution, counter)], count, temp_set, dist_set
