...
class Instance:
    ...
    #             if node[0].cust_no == 0.0: result +=  f'(Depot, service time {node[1]}) -> ' 
    #             else: result += f'(Customer {node[0].cust_no}, service time {node[1]}) -> '
                
    #             points[node[0].cust_no] = (node[0].x, node[0].y)
    #             routes = np.append(routes, node[0].cust_no)
