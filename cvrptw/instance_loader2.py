...
class Customer:
    ...
    def __init__(self, cust_no, x, y, demand):
    ...
    def copy(self):
        ...
        return Customer(self.cust_no, self.x, self.y, self.demand)
    ...
    def __str__(self):
        ...
        return f'Customer NO. : {self.cust_no}; X : {self.x}; Y : {self.y}; Demand : {self.demand}; Vehichle num: {self.vehicle_num}'
...
class Vehicle:
    ...
    def serve_customer(self, customer):
        ...
        # if not (customer.ready_time <= (ceil(vincenty(customer, self)/self.speed) + self.last_service_time) <= customer.due_date) or self.capacity <= customer.demand:
        #     return False
        if not self.capacity <= customer.demand:
            return False
    ...
    #         self.last_service_time = customer.ready_time - ceil(vincenty(customer, self)/self.speed)
...
# Kết thúc tại kho
    def return_home(self):
        if self.x != self.depo.x or self.y != self.depo.y:
            ...
            # self.last_service_time += ceil(vincenty(self, self.depo)/self.speed)
            # self.service_route += [(self.depo, self.last_service_time)]
            self.service_route += [(self.depo)]
    ...
    # xe đi đến khách hàng nào thì phải rời khỏi khách hàng đó
        ...
        for i, (curr_customer) in enumerate(self.service_route[customer_idx :]):
            prev_customer = self.service_route[customer_idx + i - 1]
            # new_time = prev_time + prev_customer.service_time + ceil(vincenty(prev_customer, curr_customer)/self.speed)
            # new_time = max(new_time, curr_customer.ready_time)
            # self.service_route[customer_idx + i] = (curr_customer, new_time)
            self.service_route[customer_idx + i] = (curr_customer)
            
        next_customer = self.service_route[customer_idx][0]
        prev_customer = self.service_route[customer_idx - 1][0]
        self.total_distance -= (vincenty(customer, next_customer) + vincenty(customer, prev_customer))
    ...
    def __str__(self):
        return f'self.id = {self.id}; x={self.x}; y={self.y}; capacity={self.capacity}; {self.service_route}'
...
class Instance:
    def __init__(self, num_vehicles, capacity, customer_list):
        ...
        assert (
            ...
            customer_list[0].cust_no == 0 and customer_list[0].demand == 0
        ...
        self.customer_list = [customer_list[0]] + sorted(customer_list[1:])
    ...
    def get_output(self, f=None):
        ...
        print("Number vehicle:", i-1, file=f)
